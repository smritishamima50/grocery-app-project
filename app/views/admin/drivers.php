...
function updateStatistics(drivers) {
    try {
        ...
        const bikeCount = drivers.filter(d => d.vehicle_type === 'bike').length;
        const carVanCount = drivers.filter(d => d.vehicle_type === 'car' || d.vehicle_type === 'van').length;
        ...
        if (bikeCountEl) bikeCountEl.textContent = bikeCount;
        if (carCountEl) carCountEl.textContent = carVanCount;
        
        console.log(`📊 Statistics updated: ${activeCount} active, ${bikeCount} bike, ${carVanCount} car/van`);
    } catch (error) {
...
async function addDriver(event) {
    ...
    const driverData = {
        ...
        vehicle_type: formData.get('vehicle_type') || 'bike',
